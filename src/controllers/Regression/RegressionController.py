...
from sklearn import linear_model #Provicional
class RegressionController(Observer):
    def __init__(self,model,view):
        super().__init__(model)
        self.model=model
        self.view=view
        self.clasifier = linear_model.LogisticRegression()
        #Agregando widget de configuración
        self.conf=RegressionConfWidget(self.view)
        self.ctl_conf=RegressionConfController(self.model,self.conf,self.clasifier)
        self.view.widget_list.addWidget(self.conf)
        #Agregando widget de predicción
        self.prediction=PredictionWidget(self.view)
        self.ctl_prediction=PredictionController(self.model,self.prediction,self.clasifier)
    ...
    def show_prediction(self):
        print("Calculando y mostrando prediccion")
